...
payloads, 9 have a canonical `tools/` source file *and* a dedicated `PayloadSync` unit test that
asserts the embedded base64 is byte-for-byte in sync with that source (`HP_COLLECT_SUBMODULES` ->
`tools/collect_submodules.py`, `HP_DETECT_PY` -> `tools/detect_python.py`, `HP_DETECT_VISA` ->
`tools/detect_visa.py`, `HP_EMBED_EXTRACT` -> `tools/embed_extract.ps1`, `HP_EMBED_PYVER_CHECK` ->
`tools/embed_pyver_check.py`, `HP_FIND_ENTRY` -> `tools/find_entry.py`, `HP_HIDDEN_IMPORT_SCAN` ->
`tools/hidden_import_scan.py`, `HP_PARSE_WARN` -> `tools/parse_warn.py`, `HP_PYPROJ_DEPS` ->
`tools/pyproj_deps.py`; each is exercised by the matching `tests/test_*.py` file). The remaining 7
(`HP_CONDARC`, `HP_DEP_CHECK`, `HP_ENV_STATE`, `HP_FAILFAST_PROBE`, `HP_FAST_CHECK`,
...
just not against a separate source file); `HP_DEP_CHECK`, `HP_ENV_STATE`, and `HP_FAILFAST_PROBE`
...
logic-test pattern as the 9 above. Any *new* embedded payload should default to the
