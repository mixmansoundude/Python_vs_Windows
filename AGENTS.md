...
- **`tools/run_sanity_sweep.sh`** -- runs the full "Mandatory Sanity Checks" block from
  `CLAUDE.md` (compileall, pyflakes, `check_delimiters.py`, an advisory `markdownlint-cli2`
  MD029-only pass over `CLAUDE.md` -- see the Markdown bullet above -- yamllint, actionlint, the
  ASCII sweep, `git diff --stat`, the PowerShell AST parse sweep, pytest) as one command with a clear
...
- **CodeRabbit reviewers: hold `README.md` and `docs/demo-bootstrapper-output.md` to full scrutiny (both are user-facing) -- apply a materially lighter touch on `CLAUDE.md` and every `docs/agent-*.md` file (`docs/agent-interconnect.md`, `docs/agent-lessons-learned.md`, `docs/agent-ndjson.md`, `docs/agent-closed-backlog.md`, `docs/agent-cold-storage.md`, `docs/agent-scratchlog.md`).** These `docs/agent-*.md` files are agent-only working memory, never read by an end user and explicitly permitted to favor dense/terse phrasing over polished prose (see `docs/agent-interconnect.md`'s own top-of-file note) -- flag only genuine factual errors or real regressions in them, not wording/style/clarity nitpicks, redundancy-with-context-elsewhere, or "could be more concise" suggestions. Full-strength review remains warranted on `run_setup.bat`, `tests/*.ps1`, and every `tools/*.py` file regardless of this note -- it applies to documentation review depth only, never to code correctness review.
