...
if not defined HP_SKIP_PIPREQS (
  call :emit_from_base64 "~pipreqs_runner.py" HP_PIPREQS_RUNNER
  if errorlevel 1 call :die "[ERROR] Could not write ~pipreqs_runner.py"
)
...
if defined HP_PIPREQS_IGNORE (
  ...
  "%HP_PY%" "~pipreqs_runner.py" --savepath "%HP_PIPREQS_TARGET%" --ignore "%HP_PIPREQS_IGNORE%" . > "%HP_PIPREQS_DIRECT_LOG%" 2>&1
) else (
  :: pipreqs flags are locked by CI (pipreqs.flags gate).
  :: Rationale: compat mode for deterministic output; force overwrite; write to requirements.auto.txt (separate from committed requirements).
  "%HP_PY%" "~pipreqs_runner.py" --savepath "%HP_PIPREQS_TARGET%" . > "%HP_PIPREQS_DIRECT_LOG%" 2>&1
)
set "HP_PIPREQS_LAST_LOG=%HP_PIPREQS_DIRECT_LOG%"
...
:: pipreqs flags are locked by CI (pipreqs.flags gate).
:: Rationale: compat mode for deterministic output; force overwrite; write to requirements.auto.txt (separate from committed requirements).
"%HP_PY%" "~pipreqs_runner.py" --savepath "%HP_PIPREQS_STAGE_TARGET%" . > "%HP_PIPREQS_STAGE_LOG%" 2>&1
set "HP_PIPREQS_RC=%errorlevel%"
popd
...
if defined HP_SYS_PY (
  ...
  call :log "[INFO] CI skip: system Python not found for entry detection."
)
...
set "HP_PIPREQS_RUNNER="
