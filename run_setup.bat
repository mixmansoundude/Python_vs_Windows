...
call :emit_from_base64 "~run_pipreqs.py" HP_RUN_PIPREQS
...
if defined HP_PIPREQS_IGNORE (
  ...
  :: NOTE: Run pipreqs via helper to avoid __main__ regressions (per CI determinism spec).
  "%HP_PY%" "%HP_WORKSPACE%\~run_pipreqs.py" . --force --mode compat --savepath "%HP_PIPREQS_TARGET%" --ignore %HP_PIPREQS_IGNORE% > "%HP_PIPREQS_DIRECT_LOG%" 2>&1
) else (
  :: pipreqs flags are locked by CI (pipreqs.flags gate).
  :: Rationale: compat mode for deterministic output; force overwrite; write to requirements.auto.txt (separate from committed requirements).
  :: NOTE: Run pipreqs via helper to avoid __main__ regressions (per CI determinism spec).
  "%HP_PY%" "%HP_WORKSPACE%\~run_pipreqs.py" . --force --mode compat --savepath "%HP_PIPREQS_TARGET%" > "%HP_PIPREQS_DIRECT_LOG%" 2>&1
)
set "HP_PIPREQS_LAST_LOG=%HP_PIPREQS_DIRECT_LOG%"
...
:: Rationale: compat mode for deterministic output; force overwrite; write to requirements.auto.txt (separate from committed requirements).
:: NOTE: Run pipreqs via helper to avoid __main__ regressions (per CI determinism spec).
"%HP_PY%" "%HP_WORKSPACE%\~run_pipreqs.py" . --force --mode compat --savepath "%HP_PIPREQS_STAGE_TARGET%" > "%HP_PIPREQS_STAGE_LOG%" 2>&1
set "HP_PIPREQS_RC=%errorlevel%"
popd
...
set "HP_RUN_PIPREQS="
