...
if /I "%HP_CHECKPOINT_SITE%"=="exe" call :log "[INFO] REQ-018: note -- this diagnostic run uses the interpreter, not the packaged EXE, so behavior can differ (e.g. bundled resources)."
...
rem CLAUDE.md Active Backlog Item 38: this internal verification of the elective Nuitka-optimized
rem build still runs from dist\ (current working directory (CWD) = dist), unlike
rem :run_exe_smokerun's own primary verification (now app root, see that subroutine's own
rem comment). Deliberately left as a documented, deferred follow-up rather than folded into this
rem slice -- no existing test depends on this specific CWD either way (selfapps_optimized_build.ps1's
rem stub app does no file I/O at all), and this is a narrower-blast-radius, opt-in-only code path,
rem not the widely-hit primary verification path Item 38 was actually about. Re-derive whether to
rem unify this too the next time this subroutine is touched.
pushd dist
...
set "HP_EXE_EXIT="
rem CLAUDE.md Active Backlog Item 38: this --hidden-import auto-recovery diagnostic re-run still
rem runs from dist\ (current working directory (CWD) = dist), unlike :run_exe_smokerun's own
rem primary verification (now app root). Deliberately left as a documented, deferred follow-up --
rem the write (~exe_out.txt, inside dist\) and the later read-back (explicit "dist\~exe_out.txt")
rem a few lines below are already internally consistent with each other regardless of which CWD is
rem chosen, and no test depends on this specific CWD (selfapps_hidden_import*.ps1's stub apps
rem locate their own token file via sys.argv[0], not a CWD-relative path, so they are unaffected
rem either way -- see docs/agent-interconnect.md "Single-verification smoke model"). Re-derive
rem whether to unify this too the next time this subroutine is touched.
pushd dist
...
set "HP_EXE_EXIT=-1"
rem CLAUDE.md Active Backlog Item 38 (resolved): the EXE smoke now runs from the app root (the
rem same current working directory (CWD) as :try_fast_exe/:verify_no_exe_interpreter and as the
rem interpreter's own run), not dist\ -- previously this was the ONE verification point using a
rem different CWD than every other one, so a CWD-relative-path app (e.g. open("config.json"))
rem could pass on a fresh build and fail on the very next (fast-path) run, or vice versa, with no
rem code change in between. Users place adjacent data files next to their .py source, which is the
rem app root, not a dist\ folder that does not even exist until after the build -- so app root is
rem the CWD that actually matches how a real double-clicked EXE finds its own adjacent files, not
rem just an artifact of how PyInstaller happens to lay out its output. Capture the EXE stdout/
rem stderr to ~run.out.txt / ~run.err.txt (now CWD-relative to the app root directly, no ..\
rem prefix needed) so the single EXE verification produces the run artifacts the old interpreter
rem smoke did -- the main 'smoke-ok' token is on stdout. Note: an app that writes a file next to
rem sys.argv[0] (e.g. the spaced-path test's ~smoke_token.txt) still writes it into dist\ for a
rem frozen EXE regardless of launch CWD (a separate, CWD-independent mechanism), so
rem selfapps_envsmoke.ps1 still reads that specific token from dist\.
rem
...
rem CLAUDE.md Active Backlog Item 38 (resolved): this diagnostic re-run now shares the SAME
rem current working directory (CWD) as :run_exe_smokerun's own primary run (the app root, not
rem dist\) -- this rerun exists purely to explain why the primary run just failed, so it must
rem reproduce the exact same launch conditions the primary run used, or the hint could describe a
rem failure the primary run never actually hit (or miss one it did).
if not exist "dist\%ENVNAME%.exe" exit /b 0
