...
rem Derived requirement: deterministic pipreqs invocation avoiding python -m __main__ quirks (compat mode contract).
call :emit_from_base64 "~pipreqs_driver.py" HP_PIPREQS_DRIVER
if errorlevel 1 call :die "[ERROR] Could not write ~pipreqs_driver.py"
set "HP_PIPREQS_DRIVER_PATH=%CD%\~pipreqs_driver.py"
...
if defined HP_PIPREQS_IGNORE (
  "%HP_PY%" "%HP_PIPREQS_DRIVER_PATH%" "%HP_PIPREQS_TARGET%" "%HP_PIPREQS_IGNORE%" > "%HP_PIPREQS_DIRECT_LOG%" 2>&1
) else (
  "%HP_PY%" "%HP_PIPREQS_DRIVER_PATH%" "%HP_PIPREQS_TARGET%" > "%HP_PIPREQS_DIRECT_LOG%" 2>&1
)
set "HP_PIPREQS_LAST_LOG=%HP_PIPREQS_DIRECT_LOG%"
...
:: pipreqs flags are locked by CI (pipreqs.flags gate).
:: Rationale: compat mode for deterministic output; force overwrite; write to requirements.auto.txt (separate from committed requirements).
"%HP_PY%" "%HP_PIPREQS_DRIVER_PATH%" "%HP_PIPREQS_STAGE_TARGET%" > "%HP_PIPREQS_STAGE_LOG%" 2>&1
set "HP_PIPREQS_RC=%errorlevel%"
popd
...
set "HP_PIPREQS_DRIVER="
